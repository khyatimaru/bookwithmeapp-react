...
export class RentalManageModal extends React.Component {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <button type='button' onClick={this.openModal} className='btn btn-bookwithme'>Bookings</button>
                <Modal open={this.state.open} onClose={this.closeModal} className={{modal: 'rental-booking-modal'}} little>
                    <h4 className='modal-title title'>Made Bookings</h4>
                    <div className='modal-body bookings-inner-container'>
                        { this.renderBookings(bookings) }
                    </div>
                    <div className='modal-footer'>
                        <button type='button' onClick={this.closeModal} className='btn btn-bookwithme'>Cancel</button>
                    </div>
                </Modal>
            </React.Fragment>
        )
