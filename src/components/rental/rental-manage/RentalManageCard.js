...
export class RentalManageCard extends React.Component {
    ...
    deleteRental(rentalId, index) {
        ...
        this.props.deleteRentalCb(rentalId, index);
    }
            ...
            <div className='col-md-4'>
                <div className={`card text-center ${deleteClass}`}>
                    <div className='card-block'>
                        ...
                        <Link className='btn btn-bookwithme' to={`/rentals/${rental._id}`}>Go to Rental</Link>
                        { rental.bookings && rental.bookings.length > 0 && modal }
                    </div>
                    <div className='card-footer text-muted'>
                        Created at {pretifyDate(rental.createdAt)}
                        { !wantDelete &&
                            <button className='btn btn-danger' onClick={() => { this.showDeleteMenu()}}> Delete </button>
                        }
                        { wantDelete &&
